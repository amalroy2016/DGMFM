...
import Models.DGM_ReLu_NonDim_UNSCALED_NS_quadratic_approx as current_model
import numpy as np
import time, os, sys
...
with tf.device(dev):
    ...
    points_set = tf.data.Dataset.from_tensor_slices(points_stack).batch(32).make_initializable_iterator()
...
#        steps = time.time_ns()
        file = f'./Bucket/Losses/Velocity_Loss_Fields'
        ...
        time.sleep(5)
        os.rename(wfile, f'./Bucket/Losses/Velocity_Loss_Fields-{steps}.vtu')
